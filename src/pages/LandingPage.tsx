...
export default function LandingPage() {
  ...
  function handleDemoLogin(email: string, password: string) {
    ...
    if (!success) showToast('Demo login failed. Please try again.', 'error');
  }
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500">
              Log in instantly with one of our demo accounts — no sign-up required.
            </p>
          </div>
            ...
            {DEMO_ACCOUNTS.map((account) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs opacity-50 font-mono mt-0.5">Password: {account.password}</p>
                </div>
                <Button
      ...
      <section className="py-20">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((feat) => (
              <div key={feat.title} className="p-6 rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all">
